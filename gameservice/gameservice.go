...
import (
	...
	"originserver2/gameservice/gm"
	_ "originserver2/gameservice/msghandler"
	"originserver2/gameservice/msgrouter"
	factory "originserver2/gameservice/objectfactory"
	"originserver2/gameservice/player"
	"originserver2/interfacedef"
	"time"
...
func (gs *GameService) OnInit() error {
	...
	mapCfg, ok := gs.GetServiceCfg().(map[string]interface{})
	if ok == false {
		return fmt.Errorf("gs config is error!")
	}

	url, ok := mapCfg["AreaId"]
	if url == 0 {
