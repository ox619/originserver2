...
import (
	...
	"originserver2/common/proto/rpc"
	"originserver2/common/util"
	"time"

	"github.com/duanhf2012/origin/v2/util/coroutine"

	"originserver2/common/env"
...
type CenterService struct {
	...
	priorityQueue map[int32]queue.PriorityQueue      //两组 map[AreaId]
...
func (cs *CenterService) OnInit() error {
	...
	cs.priorityQueue = make(map[int32]queue.PriorityQueue, 2)
...
func (cs *CenterService) refreshBalance(areaId int32, gameServiceNodeId string, gsName string, delta int) {
	...
	q, ok := cs.priorityQueue[areaId]
	if ok {
		q.Update(balanceInfo.item, balanceInfo, balanceInfo.item.Priority)
	} else {
		var q1 queue.PriorityQueue
		q1.Push(balanceInfo.item)
		cs.priorityQueue[areaId] = q1
	}

}

func (cs *CenterService) addUserInfo(areaId int32, uId string, status rpc.LoginStatus, gameServiceNodeId string, gsName string, bUpdate bool) {
	if bUpdate == false {
		cs.refreshBalance(areaId, gameServiceNodeId, gsName, -1)
	}

	cs.mapUserInfo[uId] = UserInfo{Status: status, GameServiceNodeId: gameServiceNodeId, GSName: gsName, AreaId: areaId}
}
...
func (cs *CenterService) delUserInfo(uId string) {
	...
	cs.refreshBalance(userInfo.AreaId, userInfo.GameServiceNodeId, userInfo.GSName, 1)
	delete(cs.mapUserInfo, uId)
}
...
func (cs *CenterService) getBestGameServiceNodeId(areaId int32) (string, string) {
	now := time.Now()
	if len(cs.mapUserInfo) >= cs.GSMaxPlayerNumbersPriority {
		return "", ""
	}

	q, ok1 := cs.priorityQueue[areaId]
	if ok1 {
		for {
			item := q.Pop()
			if item == nil {
...
func (cs *CenterService) RPC_Login(req *rpc.LoginGateCheckReq, res *rpc.LoginGateCheckRet) error {
	...
	if ok == true {
		...
		bestGameNodeId, bestGSName = cs.getBestGameServiceNodeId(req.ShowAreaId)
	}

	//3.找到GameService NodeId才需要存储
	if bestGameNodeId != "" {
		cs.addUserInfo(req.ShowAreaId, req.UserId, rpc.LoginStatus_Logining, bestGameNodeId, bestGSName, ok)
...
func (cs *CenterService) RPC_UpdateStatus(playerStatus *rpc.UpdatePlayerStatus) error {
	...
	if ok == false {
		...
		cs.addUserInfo(playerStatus.AreaId, playerStatus.UserId, playerStatus.Status, playerStatus.NodeId, playerStatus.GSName, ok)
...
func (cs *CenterService) RPC_UpdateBalance(balance *rpc.GameServiceBalance) error {
	...
	if ok == false {
		...
		q.Push(balanceInfo.item)
		cs.priorityQueue[balance.AreaId] = q
		return nil
	}
...
func (cs *CenterService) RPC_UpdateUserList(playerList *rpc.UpdatePlayerList) error {
	...
	for _, uId := range playerList.UList {
		cs.addUserInfo(playerList.AreaId, uId, rpc.LoginStatus_Logined, nodeId, playerList.GSName, false)
	}
