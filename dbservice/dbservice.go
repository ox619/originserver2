...
func (dbService *DBService) SyncDBIndex() error {
	...
	case util.DBService:
		/*var IndexKey [][]string
		var keys []string
		keys = append(keys, "Platform", "ServerID")
		IndexKey = append(IndexKey, keys)
		if err := s.EnsureIndex(dbService.dbName, collect.AccountCollectName, IndexKey, true, false, true); err != nil {
			return err
		}*/
